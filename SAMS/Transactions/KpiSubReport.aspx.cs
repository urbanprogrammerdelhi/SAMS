...
// Description      : Page Calls the KPISubReport to Show Drilldowns
// ***********************************************************************
// <copyright file="KpiSubReport.aspx.cs" company="Magnon">
...
/// <summary>
/// Class KPIAdmin_KpiSubReport.
/// </summary>
public partial class KPIAdmin_KpiSubReport : BasePage//System.Web.UI.Page
{
    /// <summary>
    ...
    private void FillgvKPIDetails(string statusNeedDataSource)
    {
        ...
        try
        {
            ...
            {
                ds = objKpi.GetKPISubDetail(CompanyCode, HrLocationCode, LocationCode, ClientCode, AsmtID, PostAutoID, Request.QueryString["Month"].ToString(), Request.QueryString["Year"].ToString(), Request.QueryString["ReportType"].ToString(), Request.QueryString["KPIType"].ToString());
    ...
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        gvKPI.AllowPaging = false;
        gvKPI.MasterTableView.AllowFilteringByColumn = false;
        lblError.Text = gvKPI.GroupPanel.Text;
        gvKPI.GroupPanel.Text = ResourceValueOfKey_Get(Request.QueryString["ReportType"].ToString());
        gvKPI.Rebind();

        foreach (GridItem item in gvKPI.MasterTableView.GetItems(new GridItemType[] { GridItemType.Pager, GridItemType.FilteringItem }))
            item.Display = false;
    ...
    /// <summary>
    /// Handles the OnItemCommand event of the gvKPI control.
    /// </summary>
    /// <param name="source">The source of the event.</param>
    /// <param name="e">The <see cref="GridCommandEventArgs"/> instance containing the event data.</param>
    protected void gvKPI_OnItemCommand(object source, GridCommandEventArgs e)
    {
        if (e.CommandName == "EnablePaging")
        {
            gvKPI.AllowPaging = true;
            gvKPI.GroupingEnabled = true;
            gvKPI.MasterTableView.AllowFilteringByColumn = true;
            gvKPI.GroupPanel.Text = lblError.Text;
            gvKPI.Rebind();
        }
    }
    ...
    /// <summary>
    /// For More DrillDown in New Screen
    /// </summary>
    /// <param name="sender">The source of the event.</param>
    /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
    protected void btnViewMore_Click(object sender, EventArgs e)
    {
        Server.Transfer("~/Transactions/KPIDrillReport.aspx", true);
    }

    /// <summary>
    /// Enable Diable the Button
    /// </summary>
    private void EnableDisable()
    {
        if (Request.QueryString["ReportType"].ToString().ToLower() == "TotalHoursPaidInMonth".ToLower() || Request.QueryString["ReportType"].ToString().ToLower() == "Valueofpayadjustmentsinmonth".ToLower() || Request.QueryString["ReportType"].ToString().ToLower() == "ValueofPaidNonBillableHours".ToLower() || Request.QueryString["ReportType"].ToString().ToLower() == "NumberofPaidNonBillableHours".ToLower() )
        {
            //lblOpenStatus.Text = "1";
