...
public partial class APS_MaxPDFReport : System.Web.UI.Page
{
    static int flag;
    static int dtflag;
    static int dtflag1;
    static string Branchcode = "UNTAPS013522";
    static string Date = "26 May 2022";
    static string ImageId = "";
    const string BaseLocationAutoID = "207";
    ...
    private bool IsReadAccess
    {
        get
        {
            try
            {
                ...
                return true;//IsReadAllowed(System.Web.HttpContext.Current.Request.Url.AbsolutePath.Remove(0, VirtualDirNameLenght));
            }
            catch (Exception ex)
    ...
    private bool IsWriteAccess
    {
        get
        {
            try
            {
                int VirtualDirNameLenght = 0;
                VirtualDirNameLenght = int.Parse(System.Web.HttpContext.Current.Request.Url.AbsolutePath.IndexOf(@"/", 1).ToString());
                return true; //IsWriteAllowed(System.Web.HttpContext.Current.Request.Url.AbsolutePath.Remove(0, VirtualDirNameLenght));
            }
            catch (Exception ex)
            { throw new Exception("Have not Rights", ex); }
        }
    ...
    private bool IsModifyAccess
    {
        get
        {
            try
            {
                int VirtualDirNameLenght = 0;
                VirtualDirNameLenght = int.Parse(System.Web.HttpContext.Current.Request.Url.AbsolutePath.IndexOf(@"/", 1).ToString());
                return true; //IsModifyAllowed(System.Web.HttpContext.Current.Request.Url.AbsolutePath.Remove(0, VirtualDirNameLenght));
            }
            catch (Exception ex)
            { throw new Exception("Have not Rights", ex); }
        }
    ...
    private bool IsDeleteAccess
    {
        get
        {
            try
            {
                int VirtualDirNameLenght = 0;
                VirtualDirNameLenght = int.Parse(System.Web.HttpContext.Current.Request.Url.AbsolutePath.IndexOf(@"/", 1).ToString());
                return true; //IsDeleteAllowed(System.Web.HttpContext.Current.Request.Url.AbsolutePath.Remove(0, VirtualDirNameLenght));
            }
            catch (Exception ex)
            { throw new Exception("Have not Rights", ex); }
        }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (IsReadAccess == true)
            {
                ...
                FillgvAttendence(Branchcode, Date);
    ...
    protected void FillgvAttendence(string BranchCode, string Date)
    {
        ...
        //    {
        //        ds.Tables[1].Rows[i]["ImageBase64String"] = "data:image/png;base64," + Convert.ToBase64String(((byte[])((object)ds.Tables[1].Rows[i]["ChecklistImage"])), 0, ((byte[])((object)ds.Tables[1].Rows[i]["ChecklistImage"])).Length);
        //    }
        //    else
        //    {
    ...
    protected void btnExporttoPDF_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //int counter = 0;
            //var dt = ViewState["gvAttendence"] as DataTable;
            //foreach (GridViewRow dr in gvAttendence.Rows)
            //{
            //    if (dr.RowType == DataControlRowType.DataRow)
            //    {
            //        System.Web.UI.WebControls.Image imb = dr.FindControl("imgEmpImage") as System.Web.UI.WebControls.Image;
            //        if (dt.Rows[counter]["ChecklistImage"] != DBNull.Value)
            //        {
            //            imb.ImageUrl = Convert.ToBase64String((Byte[])dt.Rows[counter]["ChecklistImage"]);
        ...
        //        if (dr.RowType == DataControlRowType.DataRow)
        //        {
        //            ImageButton imb = dr.FindControl("imgEmpImage") as ImageButton;
        //            if (!string.IsNullOrEmpty(imb.ImageUrl))
        //                imb.ImageUrl = "data:Image/png;base64," + imb.ImageUrl;
        //        }
        //    }
        //}
