...
function RecenterAndZoom(bRecenter, result) {
    if (bRecenter) {
        ...
        //var iZoomLevel = map.getBoundsZoomLevel(bounds); //manish
        //var point = bounds.getCenter(); //manish
        ...
        if (!AreMarkersWithinMapBounds()) {
            //map.setZoom(iZoomLevel);//manish
            if (cnt > 0) {
                map.fitBounds(bounds);
                //map.panToBounds(bounds);//manish
                //map.setZoom(map.getZoom() + 1);
            }
            //map.setCenter(point);//manish
        }
    }
}
